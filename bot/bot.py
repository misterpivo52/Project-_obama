...
TOKEN = 'MTQzNzA0MDkyNzkzOTEwNDc2OA.Gwa82x.WrTi5TiMEIAHbJ3XlMz_MCe4erGzb8X2EnrV-A'
...
async def enable_2fa(ctx):
    ...
    try:
        ...
        if user.two_factor_enabled:
            await ctx.send(f'{ctx.author.mention} ⚠️ Two-factor authentication is already enabled!')
            return
        ...
        embed.add_field(
            name='How it works',
            value='When you log in from the website, you will receive a verification code here in Discord with location and IP information.',
            inline=False
        )
        embed.set_footer(text='Use !disable2fa to turn off two-factor authentication')
...
async def disable_2fa(ctx):
    ...
    try:
        ...
        if not user.two_factor_enabled:
            await ctx.send(f'{ctx.author.mention} ⚠️ Two-factor authentication is already disabled!')
            return
...
async def help_command(ctx):
    ...
    embed.add_field(
        ...
        value='Register a new account\nExample: `!register test@test.com pass123 Ivan Petrov Ukraine +380123456789`',
        inline=False
    )
    embed.add_field(
        name='!login <email> <password>',
        value='Login to your account\nExample: `!login test@test.com pass123`',
        inline=False
    )
    embed.add_field(
        name='!enable2fa',
...
async def unlink_discord(ctx):
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description='Your Discord account has been unlinked from your user account.',
            color=discord.Color.orange()
        )
        embed.add_field(
            name='⚠️ Important',
            value='- Two-factor authentication has been disabled\n- You can link your Discord again by using !login',
            inline=False
        )
...
async def send_dm_async(discord_id, code, email, ip, location):
    try:
        ...
        embed.add_field(name='⚠️ Important',
                        value='This code expires in 5 minutes. If this wasn\'t you, ignore this message and change your password immediately!',
                        inline=False)
